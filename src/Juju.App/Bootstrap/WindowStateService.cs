...
public sealed class WindowStateService(IAtomicFileWriter writer)
{
    ...
    public void Restore(Window window, string key)
    {
        ...
        var area = Screen.AllScreens.Select(screen => screen.WorkingArea).FirstOrDefault(screen =>
            screen.IntersectsWith(new System.Drawing.Rectangle((int)state.Left, (int)state.Top, (int)state.Width,
                (int)state.Height)));
        if (area == default) return;
        var visible = System.Drawing.Rectangle.Intersect(area,
            new System.Drawing.Rectangle((int)state.Left, (int)state.Top, (int)state.Width, (int)state.Height));
        ...
        CancellationToken cancellationToken)
    {
        foreach (var (key, window) in windows)
        {
            ...
            _placements[key] = new(window.RestoreBounds.Left, window.RestoreBounds.Top, window.RestoreBounds.Width,
