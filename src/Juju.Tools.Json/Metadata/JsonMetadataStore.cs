...
public sealed class JsonMetadataStore(string dataRoot, IAtomicFileWriter writer)
{
    ...
    public async Task<JsonMetadata> ReadAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var metadata = JsonSerializer.Deserialize<JsonMetadata>(await File.ReadAllTextAsync(Path, cancellationToken), Options);
            return metadata is { SchemaVersion: 1 } ? metadata : throw new JsonException("Unsupported metadata schema.");
        }
    ...
    public async Task UpdateAsync(Func<JsonMetadata, JsonMetadata> update, CancellationToken cancellationToken = default)
    {
        await _gate.WaitAsync(cancellationToken);
        try { await writer.WriteTextAsync(Path, JsonSerializer.Serialize(update(await ReadAsync(cancellationToken)), Options), cancellationToken); }
