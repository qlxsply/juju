...
public sealed class StorageManager : IStorageManager
{
    ...
    public StorageManager(DataRootService roots, IAtomicFileWriter writer) { _roots = roots; _writer = writer; }
    public string? DataRoot { get; private set; }
    public bool IsAvailable { get; private set; }
    public event EventHandler<StorageExternalChange>? ExternalChanged;

    public Task InitializeAsync(string dataRoot, CancellationToken cancellationToken = default) => SwitchAsync(dataRoot, true, cancellationToken);
    public Task UseExistingDataRootAsync(string dataRoot, CancellationToken cancellationToken = default) => SwitchAsync(dataRoot, true, cancellationToken);

    public async Task MigrateToAsync(string destination, CancellationToken cancellationToken = default)
    {
        var source = DataRoot ?? throw new JujuException(ErrorCode.DataRootUnavailable, "No data root is available to migrate.");
        var target = await _roots.ValidateAsync(destination, false, cancellationToken);
        if (target.Path is null || (Directory.Exists(target.Path) && Directory.EnumerateFileSystemEntries(target.Path).Any()))
        ...
        if (target.Path.StartsWith(source + Path.DirectorySeparatorChar, StringComparison.OrdinalIgnoreCase))
            throw new JujuException(ErrorCode.DataRootInvalid, "The migration destination cannot be inside the source data root.");
        ...
        try
        {
            foreach (var file in Directory.EnumerateFiles(source, "*", SearchOption.AllDirectories))
            {
                ...
                await using (var output = new FileStream(copy, FileMode.CreateNew, FileAccess.Write, FileShare.None, 81920, FileOptions.Asynchronous | FileOptions.WriteThrough))
                {
                ...
                if (new FileInfo(file).Length != new FileInfo(copy).Length || !CryptographicOperations.FixedTimeEquals(sourceHash, copyHash))
                    throw new IOException("Copied file does not match source.");
            }
            await SwitchAsync(target.Path, false, cancellationToken);
        }
        catch { throw; } // Source is intentionally retained after failed or successful migration.
    }
    ...
    {
        try { return await File.ReadAllTextAsync(GetPath(relativePath), cancellationToken); }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException) { IsAvailable = false; throw new JujuException(ErrorCode.StorageIoError, "Storage could not be read.", ex); }
    }
    ...
    public async Task WriteTextAsync(string relativePath, string content, CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException) { IsAvailable = false; throw new JujuException(ErrorCode.StorageIoError, "Storage could not be written.", ex); }
    }
    ...
    private async Task SwitchAsync(string root, bool initializeEmpty, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!validated.IsValid) throw new JujuException(validated.Path is null ? ErrorCode.DataRootUnavailable : ErrorCode.DataRootInvalid, validated.Error ?? "The data root is invalid.");
            _watcher?.Dispose();
            DataRoot = validated.Path!;
            IsAvailable = true;
            var documents = GetPath("json", "documents");
            _watcher = new FileSystemWatcher(documents, "*.json") { NotifyFilter = NotifyFilters.FileName | NotifyFilters.LastWrite | NotifyFilters.Size, IncludeSubdirectories = false, EnableRaisingEvents = true };
            _watcher.Changed += OnWatcher; _watcher.Created += OnWatcher; _watcher.Deleted += OnWatcher; _watcher.Renamed += OnRenamed;
        }
    ...
    private void PublishChanges()
    {
        ...
        foreach (var entry in _selfWrites.Where(entry => entry.Value < DateTimeOffset.UtcNow).ToArray()) _selfWrites.TryRemove(entry.Key, out _);
    }
    ...
    public ValueTask DisposeAsync() { _watcher?.Dispose(); _debounce?.Dispose(); _gate.Dispose(); return ValueTask.CompletedTask; }
}
