...
public sealed class SettingsService(IAtomicFileWriter writer) : ISettingsService
{
    ...
    {
        if (!File.Exists(_path)) { await SaveAsync(Current, cancellationToken); return; }
        var loaded = JsonSerializer.Deserialize<AppSettings>(await File.ReadAllTextAsync(_path, cancellationToken), Options);
