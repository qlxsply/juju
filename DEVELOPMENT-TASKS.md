...
- 已完成：检查 `juju.png`、`json.png`，均为 1254×1254、32 位透明 PNG。
- 已完成：通过 `scripts/Generate-Icons.ps1` 生成 `src/Juju.App/Assets/juju.ico`、`json.ico`，包含 16/20/24/32/40/48/64/128/256 像素，保留比例和透明背景。
- 已完成：应用 EXE、托盘、Launcher、设置窗口使用 juju 图标；JSON 窗口使用 JSON 图标，标题文字置空并保留无障碍名称。
- 验证：两套 ICO 的 9 个尺寸均通过 WPF 解码及透明度检查；16～128 像素通过 System.Drawing.Icon 解码检查；App 隔离输出构建成功（0 编译警告、0 错误）。实际桌面显示效果待运行查看。
